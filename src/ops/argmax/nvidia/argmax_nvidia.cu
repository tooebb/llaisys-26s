...
// 每 block 计算局部最大值，存到 partial_max_idx / partial_max_val
template <typename T>
...
void launch_argmax(std::byte *max_idx, std::byte *max_val,
    ...
    // 用临时 buffer 存每个 block 的局部结果
    int64_t *d_partial_idx;
    T *d_partial_val;
    cudaMalloc(&d_partial_idx, grid * sizeof(int64_t));
    cudaMalloc(&d_partial_val, grid * sizeof(T));

    argmax_reduce_kernel<T><<<grid, BLOCK_SIZE>>>(
        (const T *)vals, numel, d_partial_idx, d_partial_val);

    // 第二次 reduce（block 数量少，直接在一个 block 内完成）
